          ...
          <div class="ripple-wave">
            <div class="ripple-wave-2">
              <img
                class="avatar"
                :src="
                  '//app.nihaoshijie.com.cn/upload/avatar/avatar' +
                    othersInfo.avatar
                "
              />
            </div>
        ...
        <div class="ripple-wave">
          <div class="ripple-wave-2">
            <img
              class="avatar"
              :src="
                '//app.nihaoshijie.com.cn/upload/avatar/avatar' + info.avatar
              "
            />
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      full: false, // 房间满人标志位
      pcConfig: {
        // webrtc建立点对点的对等连接需要用到iceServers，否则只能在内网局域网使用
        iceServers: [
          {
  ...
  /**A 拨打端   B为接受端
   * 1.点击拨打按钮（只存在某个用户(客户页面只对某人)界面指定对某人进行拨打）
   * 2.创建 rtc实例并创建offer(SDP),发送至服务端（socket发送）   // 创建offer的时候需要 ice服务器实现点对点连接
   * 3.B端 显示接受拒绝操作按钮
   *    (1).拒绝：发送socket事件B拒绝接受   同时A端接受拒绝事件做出相对操作（被拒绝通知关闭流和rtc操作）
   *    (2).同意：接受A发送的offer,自身创建rtc以及连接点 并把A的offer加入连接点和获取流操作，再把B自己的创建的offer 发送到服务器，A接受后也把B的offer 加入连接点
   *     // 最后把 候选对象添加入 rtc对象 实现实时通信
   *    (3).超时
   *    (4) 当呼叫人正在通话中（怎么解？）
   * 4.获取双方的流进行标签播放
   */
